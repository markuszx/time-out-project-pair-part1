...
function intervalCount(cb, delay, amount) {
  // Your code here
  //count variable
//   let setInt = setInterval(cb, delay)
//  let count =0
//   const intervalObj = setInterval(cb,delay,function (){
//   count++
// if(count === amount) {clearInterval(intervalObj)};
...
let setInt = setInterval(function(){
  cb();
  amount--;
  if(amount === 0){clearInterval(setInt)}
}, delay)


// if(cb){ count++}
//   //if count === amount clearInterval
// count < amount ? setInterval(cb, delay) : clearInterval(setInt)
